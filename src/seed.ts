...
export function seedDatabase(
  ...
  console.log(`📊 Generated ${clientCount} clients, ${bitscamCount} bitscams, and ${transactionCount} transactions.`);
 ...
 */
function seedBitscams(db: Database, count: number, clientCount: number): number[] {
  ...
  db.transaction(() => {
    for (let i = 0; i < count; i++) {
      ...
      const clientId = Math.random() > 0.2 
...
function generateDistinctRandomValues(count: number, min: number, max: number): number[] {
  if (max - min + 1 < count) {
    throw new Error(`Cannot generate ${count} distinct values in range [${min}, ${max}]`);
  }
 ...
 */
function seedTransactions(db: Database, count: number, bitscamCount: number, clientCount: number) {
  ...
  const bitscamOwners: Record<number, number | null> = {};
    ...
    db.transaction(() => {
        for (let i = 0; i < count; i++) {
            ...
            // Get bitscam value
            const bitscamValue = db.query(
                "SELECT value FROM bitscams WHERE id = ?"
            ).get(bitscamId)?.value || 0;
            ...
            latestTransactionDate = new Date(latestTransactionDate.getTime() + minutesToAdd * 60000);
